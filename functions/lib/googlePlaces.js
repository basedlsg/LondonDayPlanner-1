...
const new_york_areas_1 = require("../data/new-york-areas");
...
function convertNewPlaceToLegacy(newPlaceData) {
    var _a, _b, _c;
    return {
        name: ((_a = newPlaceData.displayName) === null || _a === void 0 ? void 0 : _a.text) || '',
        formatted_address: newPlaceData.formattedAddress || '',
        geometry: {
            location: {
                lat: ((_b = newPlaceData.location) === null || _b === void 0 ? void 0 : _b.latitude) || 0,
                lng: ((_c = newPlaceData.location) === null || _c === void 0 ? void 0 : _c.longitude) || 0
            }
        },
...
function convertNewSearchResultsToLegacy(searchResponse) {
    if (!searchResponse.places || !Array.isArray(searchResponse.places)) {
        return { results: [], status: "ZERO_RESULTS" };
    }
    const results = searchResponse.places.map((place) => {
        var _a, _b, _c;
        return ({
            place_id: place.id || '',
            ...
            geometry: {
                location: {
                    lat: ((_b = place.location) === null || _b === void 0 ? void 0 : _b.latitude) || 0,
                    lng: ((_c = place.location) === null || _c === void 0 ? void 0 : _c.longitude) || 0
                }
            },
...
async function fetchPlaceDetails(placeId, includeReviews = false) {
 ...
 */
async function checkPlaceReviewsForKeywords(placeId, keywords) {
    try {
        // Get detailed place information including reviews
        const details = await fetchPlaceDetails(placeId, true);
        if (!(details === null || details === void 0 ? void 0 : details.reviews) || !Array.isArray(details.reviews)) {
            return false;
        }
        // Check if any keywords appear in review text
        return details.reviews.some((review) => {
            if (!review.text)
                return false;
...
async function searchPlace(query, options = {}) {
    ...
    try {
        ...
        const matchingArea = new_york_areas_1.nycAreas.find(area => area.name.toLowerCase() === query.toLowerCase() ||
            area.neighbors.some(n => n.toLowerCase() === query.toLowerCase()));
        // Normalize the location name
        const normalizedLocation = (0, locationNormalizer_1.normalizeLocationName)(query);
        ...
        if (options.type && options.type !== "landmark") {
            ...
            if (convertedLandmarkData.status !== "OK" || !((_a = convertedLandmarkData.results) === null || _a === void 0 ? void 0 : _a.length)) {
                const suggestions = (0, locationNormalizer_1.suggestSimilarLocations)(query);
                throw new Error(`Could not find "${query}"${suggestions.length ? `. Did you mean: ${suggestions.join(", ")}?` : ""}. ` +
            ...
            if (convertedNearbyData.status !== "OK" || !((_b = convertedNearbyData.results) === null || _b === void 0 ? void 0 : _b.length)) {
                ...
                if (convertedFallbackData.status === "OK" && ((_c = convertedFallbackData.results) === null || _c === void 0 ? void 0 : _c.length) > 0) {
            ...
            if (options.minRating !== undefined) {
                ...
                const qualifiedResults = results.filter((r) => r.rating >= minRating);
            ...
            if (options.checkReviewsForKeywords && keywordsList.length > 0) {
                try {
                    ...
                    if (matchingResults.length > 0) {
                        ...
                        const nonMatchingResults = results.filter(result => !matchingResults.some(match => match.place_id === result.place_id));
            ...
            if (options.type) {
                if (options.type === 'cafe' || options.type === 'coffee') {
                    ...
                    const filteredResults = results.filter(place => !place.types.includes('gas_station') &&
                        !place.types.includes('hospital') &&
                ...
                if (results.length === 0) {
                    ...
                    if (searchKeyword || keywordsList.length > 0) {
                        ...
                        if (keywordData.status === "OK" && ((_d = keywordData.results) === null || _d === void 0 ? void 0 : _d.length) > 0) {
                            results = keywordData.results;
                            // Apply some basic filtering to these results too
                            results = results.filter(place => !place.types.includes('gas_station') &&
                    ...
                    if (results.length === 0) {
                        ...
                        if (genericData.status === "OK" && ((_e = genericData.results) === null || _e === void 0 ? void 0 : _e.length) > 0) {
            ...
            const alternativePromises = results.slice(1).map(async (result) => {
                ...
                const distance = calculateDistance(primaryLat, primaryLng, result.geometry.location.lat, result.geometry.location.lng);
                return {
                    ...details,
            ...
            if (convertedSearchData.status !== "OK" || !((_f = convertedSearchData.results) === null || _f === void 0 ? void 0 : _f.length)) {
                const suggestions = (0, locationNormalizer_1.suggestSimilarLocations)(query);
                throw new Error(`Could not find "${query}"${suggestions.length ? `. Did you mean: ${suggestions.join(", ")}?` : ""}. ` +
                    "Try being more specific or using the full name.");
            }
            ...
            if (!(0, locationNormalizer_1.verifyPlaceMatch)(query, primaryResult.name, primaryResult.types)) {
